      SUBROUTINE degfac(m,Tobs,m1input,m2input,Ps
     &,omperinput,ideg,ec,Pod,deg_fac)       

C       IMPLICIT NONE
       real*8 Msun,pi,G,c
       real*8, intent(in) ::  m,Tobs,m1input
       real*8, intent(in) :: m2input, Ps
       real*8, intent(in) :: omperinput, ideg
       real*8, intent(in) :: ec, Pod
       real*8, intent(out) :: deg_fac
       real*8 Omo,m1,m2,omper
       real*8 aR,a,ap
       real*8 Omp
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine 
     &findalpha(xmy,Ompy,apy,ompery,ecy,Omoy,Tpy,Tob,gammaf)
       IMPLICIT NONE
...
CCCCCCCCCCCC  initial alpha2, gamma calculation over the full observation   CCCCCC
     ...
     &(1.0D0+ecy*cos(farr(i)))*(1.0D0+ecy*cos(farr(i)))
      ...
      call gam(alpha1rr(i),alpha2rr(i),rl0,Tob,gammaarr(i))
      ntagarr(i)=dble(i)
...
CCCCCCCCC
          call gam(alpha1rrn(ii),alpha2rrn(ii),rl0,Tob,gammaarrn(ii))
          ntagarrn(ii)=dble(ii)
        ENDDO
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
       SUBROUTINE gam(alpha1y,alpha2y,rl0y,t,ans)
       IMPLICIT NONE
